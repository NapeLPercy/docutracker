...
function showSection(sectionId) {
  ...
  console.log("show section id == ", sectionId);
...
function loadWorkersByRole() {
  ...
  fetch("http://localhost:3000/worker?role=" + role, {
    method: "GET",
    headers: {
      "Content-Type": "application/json",
    },
...
function getMyTasks() {
  ...
  fetch(`http://localhost:3000/task?id=${id}&role=${role}`, {
    method: "GET",
...
function getDashboard() {
  ...
  fetch(`http://localhost:3000/task/dashboard?id=${id}&role=${role}`, {
    method: "GET",
    headers: { "Content-Type": "application/json" },
  })
    .then((response) => response.json())
...
function deleteUser(persal, btnElement) {
  ...
  fetch(`http://localhost:3000/api/users/${persal}`, {
    method: "DELETE",
  })
    .then((res) => {
